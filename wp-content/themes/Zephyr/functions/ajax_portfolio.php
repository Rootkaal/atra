...
if ( ! function_exists('portfolioAjaxPagination'))
{
	function portfolioAjaxPagination(){
		...
		while ($wp_query->have_posts())
		{
			...
			$output .= '<div class="w-portfolio-item order_'.$item_categories_classes.' animate_reveal">
							<div class="w-portfolio-item-h">
								<a class="w-portfolio-item-anchor"'.$link_target.$link_ref.' href="'.esc_url_raw($link).'"'.$anchor_css.'>
									<div class="w-portfolio-item-image">';
			if (has_post_thumbnail()) {
				$output .= get_the_post_thumbnail(null, 'portfolio-list-'.$ratio, array('class' => 'w-portfolio-item-image-first'));
			} else {
				$output .= '<img class="w-portfolio-item-image-first" src="'.esc_url(get_template_directory_uri().'/img/placeholder/500x500.gif').' alt="">';
			}
