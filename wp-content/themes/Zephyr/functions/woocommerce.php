...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	?>
	<a class="cart-contents" href="<?php echo esc_url($woocommerce->cart->get_cart_url()); ?>" title="<?php _e('View your shopping cart', 'woothemes'); ?>"><?php echo sprintf(_n('%d item', '%d items', $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count);?> - <?php echo $woocommerce->cart->get_cart_total(); ?></a>
	<?php
...
global $smof_data;
if ( ! empty($smof_data['products_listing_style']) AND $smof_data['products_listing_style'] == 'Card style')
{
	define('WOO_STYLING_CLASS', 'woocommerce-type_2');
} else {
	define('WOO_STYLING_CLASS', 'woocommerce-type_1');
}