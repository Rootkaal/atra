...
function us_styles()
{
	...
	wp_register_style('us-font-mdfi', $template_directory_uri.'/css/font-mdfi.css', array(), '1', 'all');
...
function us_custom_styles()
{
	...
	// Migrate options and resave theme custom CSS if the theme was updated
	$theme_version = us_get_main_theme_version();
	...
	if ( empty( $last_custom_css_version ) OR version_compare( $last_custom_css_version, $theme_version, '<' ) ) {
		of_migrate_options( $smof_data, $last_custom_css_version );
	...
	$styles_file = $styles_dir.'/us_zephyr_custom_styles.css';

	if ( get_template_directory_uri() != get_stylesheet_directory_uri() ) {
		wp_register_style( 'theme-style', get_stylesheet_directory_uri() . '/style.css', array(), $theme_version, 'all' );
		wp_enqueue_style( 'theme-style' );
	}

	if (file_exists($styles_file)){
		wp_register_style('us_custom_css', $wp_upload_dir['baseurl'] . '/us_custom_css/us_zephyr_custom_styles.css', array(), $theme_version, 'all');
...
function us_jscripts()
{
	...
	wp_register_script('us-mediaelement', $template_directory_url.'/js/mediaelement-and-player.min.js', array('jquery'), '', TRUE);
