...
class US_Shortcodes {
	...
	public function vc_social_links($attributes, $content = null)
	{
		...
		$output = '<div class="w-socials'.$size_class.$align_class.$inverted_class.$desaturated_class.$attributes['el_class'].'">
			<div class="w-socials-list">';

		foreach ($socials as $social_key => $social ) {
			if ( empty( $attributes[ $social_key ] ) ) {
				continue;
			}
			$social_url = $attributes[ $social_key ];
			if ( $social_key == 'email' ) {
	...
	public function vc_actionbox ($attributes, $content)
	{
		...
		$output = 	'<div class="w-actionbox controls_aside color_'.$attributes['type'].$attributes['el_class'].'"'.$item_style.'>'.
	...
	public function vc_iconbox($attributes, $content)
	{
		...
		$output =	'<div class="w-iconbox'.$no_text_class.$img_class.$iconpos_class.$size_class.$type_class.$color_class.$attributes['el_class'].'">
						'.$link_start.'
	...
	public function vc_testimonial($attributes, $content)
	{
		...
		$output = 	'<div class="w-testimonial'.$animate_class.$type_class.$attributes['el_class'].'">
						<blockquote>
	...
	public function vc_member ($attributes, $content)
	{
		...
		$output = 	'<div class="w-team'.$animate_class.$type_class.$img_circle_class.$social_class.$attributes['el_class'].'">
								...
								jQuery("#us_load_more_'.$rand_id.' .g-btn").click(function(){
									...
									jQuery.ajax({
										...
										success: function(data, textStatus, XMLHttpRequest){
											...
											blogContainer = jQuery("#us_blog_'.$rand_id.'"),
				...
				if ($us_thumbnail_size == 'blog-grid') {
					$output .= '
											...
											newItemsContainer.imagesLoaded(function(){
												...
												jQuery("#us_load_more_'.$rand_id.'").removeClass("loading").toggleClass("done", max_page <= page);
											});';
				} else {
					$output .= '
											var newItems = newItemsContainer.children();
	...
	public function vc_portfolio($attributes, $content)
	{
		...
		$output = '<div class="w-portfolio '.$attributes['style'].' align_'.$attributes['align'].' columns_'.$attributes['columns'].' '.
				  'ratio_'.$attributes['ratio'].$sortable_class.$with_indents_class.$attributes['el_class'].' animate_revealgrid" id="us_portfolio_'.$rand_id.'">
						'.$filters_html;
	...
	public function vc_clients($attributes, $content)
	{
		...
		$type_class = ($attributes['type'] == 'flat')?' type_2':' type_1';
		...
		$output = 	'<div class="w-clients'.$arrows_class.$type_class.$attributes['el_class'].'">
	...
	public function gallery($attributes)
	{
		...
		$indents_class = ($indents == 1 OR $indents == 'yes' OR $indents == 'true')?' with_indents':'';
