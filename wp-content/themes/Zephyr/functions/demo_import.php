...
if ( !function_exists('us_demo_import'))
{
	...
	{
		$us_demo1_url = 'http://zephyr.us-themes.com/';
		?>
		...
		<div class="w-message success" style="display:none;">
			...
			<p class="w-message-text">Now you can see the result at <a href="<?php echo esc_url(site_url()); ?>" target="_blank">your site</a><br>or start customize via <a href="<?php echo esc_url(admin_url('themes.php?page=optionsframework')); ?>">Theme Options</a>.
		</div>

		<form class="w-importer" action="?page=us_demo_import" method="post">

			<h1 class="w-importer-title">Zephyr Demo Import (<a href="<?php echo $us_demo1_url; ?>" target="_blank">preview</a>)</h1>
			...
			<div class="w-importer-list">
				...
				<div class="w-importer-item">
					...
					<div class="w-importer-item-btn">
						<a class="button" href="<?php echo $us_demo1_url; ?>" target="_blank">Preview</a>
					</div>
				</div>
				<div class="w-importer-item">
					<input class="w-importer-item-radio" id="demo2" type="radio" value="demo2" name="demo">
					<label class="w-importer-item-preview" for="demo2">
						<h2 class="w-importer-item-title">Demo Two</h2>
						<img src="<?php echo get_template_directory_uri(); ?>/functions/assets/img/demo-2.png" alt="">
					</label>
					<div class="w-importer-item-btn">
						<a class="button" href="http://zephyr1.us-themes.com/" target="_blank">Preview</a>
					</div>
				</div>
		...
		<script>
			jQuery(document).ready(function(){
				...
				jQuery('#import_demo_data').click(function(){
					...
					var demo = jQuery('input[name=demo]:checked').val() || 'demo1',
						importQueue = [],
					...
					if (jQuery('#rev_slider').is(':checked')) importQueue.push('us_demo_import_sliders');
	...
	function us_demo_import_content(){
		...
		if(!empty($menus))
		{
			...
			{
				if(is_object($menu) && $menu->name == 'Zephyr Header Menu')
				{
					$locations['zephyr_main_menu'] = $menu->term_id;
				}
			}
		}
