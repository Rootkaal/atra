...
if (in_array(rwmb_meta('us_titlebar'), array('', 'caption_only'))) {
	...
	} elseif (rwmb_meta('us_titlebar_color') == 'default') {
		$color_class = '';
	}
...
//		$titlebar_img = wpb_getImageBySize(array('attach_id' => $titlebar_img_id, 'thumb_size' => 'full' ));
		$titlebar_img = wp_get_attachment_image($titlebar_img_id, 'full', 0);
	...
	if ($titlebar_image != ''){
		...
		if ($titlebar_parallax == 'vertical' || $titlebar_parallax == 'still'){
	...
	if (rwmb_meta('us_header_layout_type') == 'Compact'){
		$header_container_layout_type = ' size_medium';
	}
	...
	?>
	<div class="l-submain for_pagehead<?php echo esc_attr($color_class.$header_container_layout_type.$parallax_class); ?>"<?php echo $section_id_string; ?>>
		...
		<div class="l-submain-h g-html i-cf">
			<div class="w-pagehead">
				...
				<div class="w-pagehead-nav">
					...
					if( ! empty($prev_item)) {
						?>
						<a class="w-pagehead-nav-item type_prev" href="<?php echo get_permalink($prev_item->ID); ?>" title="<?php echo esc_attr(get_the_title($prev_item->ID)); ?>"><i class="mdfi_navigation_chevron_left"></i></a>
						<?php
					}
					if( ! empty($next_item)) {
						?>
						<a class="w-pagehead-nav-item type_next" href="<?php echo get_permalink($next_item->ID); ?>" title="<?php echo esc_attr(get_the_title($next_item->ID)); ?>"><i class="mdfi_navigation_chevron_right"></i></a>
						<?php
					}
					?>
					</div>
